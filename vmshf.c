 ...
 *
 * Thanks to Tom Ehlert for the new definitions of Request and Reply
 * macros for smaller code
 *
...
int VMShfOpenFile(uint32_t access,      /* one of VMSHF_ACCESS_* values */
                  ...
                  char __far *filename, /* variable length file name    */
...
int VMShfReadFile(uint32_t handle,  /* 10=> file handle returned by OPEN */
...
int VMShfWriteFile(uint32_t handle,                /* 10: file handle returned by OPEN */
                   uint8_t flags, uint64_t offset, /* 15: byte offset to start writing */
...
int VMShfCloseFileDir(uint32_t op,
...
int VMShfOpenDir(char __far *dirname, uint8_t utf, /* filename is in UTF */
                 uint32_t *status, uint32_t *handle)
{
    uint32_t namelen, datalen;
    int ret;
...
int VMShfReadDir(uint32_t handle, /* 10: file handle returned by OPEN */
                 uint32_t index,
                 uint32_t *status,
                 VMShfAttr **fileattr,
                 char **filename,
...
int VMShfGetAttr(char __far *filename,
                 uint8_t utf, /* filename is in UTF */
                 uint32_t handle,
                 uint32_t *status,
                 VMShfAttr **fileattr)
...
int VMShfSetAttr(VMShfAttr *fileattr,
                 char __far *filename,
                 uint8_t utf, /* filename is in UTF */
                 uint32_t handle,
                 uint32_t *status)
...
int VMShfCreateDir(uint8_t dirmode,
                   char __far *dirname,
                   uint8_t utf, /* filename is in UTF */
                   uint32_t *status)
{
...
int VMShfDeleteFileDir(uint32_t op,
                       char __far *filename,
                       uint8_t utf, /* filename is in UTF */
                       uint32_t *status)
{
...
int VMShfMoveFile(char __far *srcname,
                  char __far *dstname,
...
int VMShfGetDirSize(char __far *dirname,
                    uint8_t utf, /* filename is in UTF */
                    uint32_t *status,
                    uint64_t *avail,
                    uint64_t *total)
