...
uint8_t manglingChars = DEF_MANGLING_CHARS;
uint8_t hashLen = DEF_MANGLING_CHARS << 2;

char longFileName1[VMSHF_MAX_PATH];
char longFileName2[VMSHF_MAX_PATH];
 ...
 *       also in the DOMAIN. (The actual code is fom the version
...
uint32_t LfnFNameHash(uint8_t *name, // in : File name in UTF-8
...
//
inline int MatchFcbNameToFileName(char *fcbName,       // in : DOS FCB name
                                  char __far *fileName // in : DOS 8.3 name
)
{
    int i = 0;
...
//
inline char __far *FindRealName(char __far *dest,     // out : destination buffer
                                char __far *path,     // in : search path
                                char __far *fileName, // in : 8.3 file name
...
)
{
    ...
    for (;;)
    {
        ...
        if (FNameToFcbName(fcbName, fName, (uint16_t)fNameLen, isRoot, 1) && MatchFcbNameToFileName(fcbName, fileName))
        {
            (void)VMShfCloseDir(handle, &status);
            return d;
        }
...
//
char __far *LfnGetTrueLongName(char __far *dest, // out : destination buffer
                               char __far *path  // in : DOS path
)
{
    char __far *s, __far *d;
    char __far *tl, __far *ps, __far *ns, savens;
    int ret, len = 0;
 ...
 */
uint16_t BeginOfTransientBlockWithLfn(void)
{
