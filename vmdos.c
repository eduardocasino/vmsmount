...
uint8_t __far *fpFUcase;
FChar __far *fpFChar;
int32_t gmtOffset = 0;
uint8_t caseSensitive = false;

int IllegalChar(unsigned char c)
{
    int i = 0;
    ...
    if ((c < fpFChar->lowest || c > fpFChar->highest) || !(c < fpFChar->firstX || c > fpFChar->lastX))
    {
        return true;
    }
...
unsigned char toupper_local(unsigned char c)
{
    if (c > 0x60 && c < 0x7b)
    {
        return c & 0xDF;
...
//
int FNameToFcbName(char *fcbName,     // out : File name in FCB format. Must be 12 bytes long (we NULL-terminate it)
                   char *fName,       // in/out : File name in in UTF-8 on input, DOS filename on output
...
uint64_t FatTimeToFTime(uint32_t fatTime)
{
    union {
        uint64_t fTime64;
        uint32_t fTime32;
...
int DosPathToPortable(uint8_t *dst, uint8_t __far *src, uint8_t utf)
{
    int len, i;

    if (src == (uint8_t __far *)0)
    {
        return -1;
    }
